...
class Config:
      DATASET_PATH = "./train"
      VAL_PATH = "./testval"
      SINGLE_TEST_PATH = "./test332/936332"
      BATCH_SIZE = 10
      EPOCHS = 1000
      MODEL_PATH = "./model/model_HSV_0.5_10_2000_mse_170_1.hdf5"#"./model_11/model_HSV_0.1_V1.hdf5"
      STATE = False
      SIZE1 = 64
      SIZE2 = 64
      SZ = 500
...
def evaluate():
        ...
        # sa = np.zeros(sz)
        # for ii in range(sz):
        #     if sequences_reconstruction_cost[ii] > 36.195543067834684:
        #         sa[ii] = 1
        #     else:
        #         sa[ii] = 0
...
def evaluate_reli():
    ...
    RELI_MATRIX = np.zeros((num,int(sz+10),Config.SEQENCE, Config.SIZE1, Config.SIZE2, Config.CHANNEL))
    sequences1 = np.zeros((int(sz+10), Config.SEQENCE, Config.SIZE1, Config.SIZE2, Config.CHANNEL))
    ...
    sequences1[sz:] = sequences2[int(sz-10):]
    ...
    #RELI_IMG = (RELI_IMG-RELI_IMG.min(axis=0))/(RELI_IMG.max(axis=0)-RELI_IMG.min(axis=0))
    ...
    RELI_IMG = RELI_IMG.reshape((int(sz+10), Config.SEQENCE, Config.SIZE1, Config.SIZE2, Config.CHANNEL))
    ...
    strname = Config.SINGLE_TEST_PATH
    str11 = strname.split('/')
    savefolder1 = str11[-1]+'_'+str(Config.DA) + '_' + str(Config.TEST_NUM) + '_' + str(timevalue)
    if not os.path.exists(savefolder1):
        os.mkdir(savefolder1)
    RELISUM = np.array([sum(RELI_IMG[i,:,:,:,].flatten()) for i in range(0,int(sz+10))])
    ...
    plt.show()
    # print(11111111111111111111111111111)
    # sa = np.zeros(RELISUM.shape[0])
    # for ii in range(RELISUM.shape[0]):
    #     if RELISUM[ii] > 423.4675439547984:
    #         sa[ii] = 1
    #     else:
    #         sa[ii] = 0
    #sa = softmax(RELISUM)
    ...
    #levels = [0.2,0.25,0.3,0.35,0.4,0.45,0.5]
    levels = [0.00,0.005,0.01,0.05,0.07,0.1,0.2,0.3,0.4]
    ...
    for iii in range(Config.SZ-10):
        #IMG11 = RELI_IMG[int(iii),0,:,:]
        #IMG11 = np.multiply(RELI_IMG[int(iii),0,:,:],MO)
        IMG11 = abs(RELI_IMG[int(iii),0,:,:] - MO)
        # print(111111111111111111111111111)
        # print(np.max(IMG11))
        # print(np.min(IMG11))
        # print(111111111111111111111111111)
        if sum(IMG11.flatten()>0.1)<130:
            IMG11[IMG11<0.38]=0.05
